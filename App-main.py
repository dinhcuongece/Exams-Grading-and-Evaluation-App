...
#PREPROCESSING
    ...
    try:
        ...
        rectCon = LocalFunction.rectContour(contours) # FILTER FOR RECTANGLE CONTOURS
        biggestPoints= LocalFunction.getCornerPoints(rectCon[0]) # GET CORNER POINTS OF THE BIGGEST RECTANGLE
        gradePoints = LocalFunction.getCornerPoints(rectCon[1]) # GET CORNER POINTS OF THE SECOND BIGGEST RECTANGLE
        ...
        if biggestPoints.size != 0 and gradePoints.size != 0:
            ...
            #TODO: Catch the paper-edge case
            biggestPoints=LocalFunction.reorder(biggestPoints) # REORDER FOR WARPING
            ...
            gradePoints = LocalFunction.reorder(gradePoints) # REORDER FOR WARPING
            ...
            # DISPLAYING ANSWERS
            LocalFunction.showAnswers(imgWarpColored, myIndex, grading, ans) # DRAW DETECTED ANSWERS
            LocalFunction.drawGrid(imgWarpColored) # DRAW GRID
            imgRawDrawings = np.zeros_like(imgWarpColored) # NEW BLANK IMAGE WITH WARP IMAGE SIZE
            LocalFunction.showAnswers(imgRawDrawings, myIndex, grading, ans) # DRAW ON NEW IMAGE
            ...
            cv2.putText(imgRawGrade,str(int(score))+"%",(70,100)
                        ,cv2.FONT_HERSHEY_COMPLEX,3,(0,0,119),10) # ADD THE GRADE TO NEW IMAGE
            ...
            imgFinal = cv2.addWeighted(imgFinal, 0.75, imgInvGradeDisplay, 1,0) #TODO: fix grade opacity
    ...
    stackedImage = LocalFunction.stackImages(imageArray, 0.5, lables)
    cv2.imshow("Result",stackedImage)


    # SAVE GRADE AND PROCESS WHEN 's' key is pressed
    if cv2.waitKey(1) & 0xFF == ord('s'):
        cv2.imwrite(os.path.join(OutputPath,'Grade')+str(count)+".jpg",imgFinal)
        cv2.imwrite(os.path.join(OutputPath,'Process')+str(count)+".jpg",stackedImage)
